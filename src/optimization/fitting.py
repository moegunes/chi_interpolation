...
def fit_params(r, rslist, model=X_r2_two_mode, inverse=False):
    from tqdm.notebook import tqdm

    parameters = {}
    parameters_cov = {}
    parameters["model"] = model
    #    global B,factor,kF,n0,NF,fxc
    if inverse:
        rslist = rslist[::-1]
    for idx_rs in tqdm(range(len(rslist)), desc="Fitting", ncols=80):
        ...
        p_opt, p_cov = guess_X(
            rs, X_exact, model, initial_guess, B, kF, factor, kFr0=0, kFr1=4
        )
        parameters[rs] = p_opt
...
def guess_X(r, rs, X_exact, model, initial_guess, B, kF, factor, kFr0=0, kFr1=8):
    fit_idx0 = np.argmin(np.abs(kF * r - kFr0))
    fit_idx1 = np.argmin(np.abs(kF * r - kFr1))

    def model_wrapper(r, gamma0, f0, phi0, gamma1, f1, phi1):
        params = [gamma0, f0, phi0, gamma1, f1, phi1]
        return model(r, B=B, rs=rs, factor=factor, params=params)
