...
from analysis.physics import canon_cos_phase, get_chi
from optimization.production import get_chi_interp2
...
def plot_parameters(params_dict):
    ...
    rsl.sort()
    rsl = rsl[4:]
    # kFl=(9*np.pi/4)**(1/3)/np.array(rsl)
    font_size = 6
    marker_size = 2
    lww = 0.7
    ...
    B0l = [
        get_constraints(params_dict[rsl[r]], rsl[r], model=params_dict["model"])[0]
        for r in range(len(rsl))
    ]
    B1l = [
        get_constraints(params_dict[rsl[r]], rsl[r], model=params_dict["model"])[1]
        for r in range(len(rsl))
    ]
...
def plot_chi(r, q, params_dict, rs, error=False):
    ...
    NF = kF / (1 * pi**2)
    chiR = get_chi(rs)
    chi_interpp = get_chi_interp2(params_dict, rs)
    dr = r[1] - r[0]
    font_size = 8
    ...
    chi0q = chi00q(q, rs, interacting=False)
