...
from utils.utils_chi import chi00q, corradini_pz, get_chi, get_chiq, get_gas_params
...
def plot_parameters(params_dict):
    ...
    rsl.sort()
    # filter where rs is larger than 0.5
    # rsl = [rs for rs in rsl if rs > 0.4]
    font_size = 6
    marker_size = 2
    lww = 0.7
    ...
    for j in range(3):
        ...
        ax[j].plot(rsl, coef1l, "k-", label=r"$m=1$", lw=lww, markersize=marker_size)
        ax[j].plot(rsl, coef2l, "r-", label=r"$m=2$", lw=lww, markersize=marker_size)
    ...
    ax[3].plot(rsl, B0l, "k-", label=r"$m=1$", lw=lww, markersize=marker_size)
    ax[3].plot(rsl, B1l, "r-", label=r"$m=2$", lw=lww, markersize=marker_size)
...
def plot_chi(r, q, params_dict, rs, error=False, ax=None):
...
def plot_chi_interpolated(r, q, params_dict, rs, fits, ax=None, show_diff=True):
    ...
    chi_orig = get_chi(q, rs)  # get_chi_interp(r, q, params_dict, rs)
    ...
    chi_interp = get_chi_interp(r, q, temp_dict, rs)

    # Fourier transforms
    FT_q_orig, FT_chiq_orig = chi_q_from_chi_r_fast(r, chi_orig, qlist=None)
    FT_q_interp, FT_chiq_interp = chi_q_from_chi_r_fast(r, chi_interp, qlist=None)
    ...
    diff_r = (chi_interp - chi_orig) / (2 * kF**4 / pi)
...
def scan_chi_interpolation_error(params_dict, form_name="Pade[2/2]", rs_test=None):
    ...
    for rs in rs_test:
        ...
        diff_r = np.abs(chi_interp - chi_orig) / norm_r
        diff_q = np.abs(FT_chiq_interp - FT_chiq_orig) / norm_q

        # Relative to original signal
        rel_r = np.max(diff_r) / (np.max(np.abs(chi_orig)) / norm_r + 1e-30) * 100
        rel_q = (
        ...
        MADE_e = np.sum(diff_r) / (np.sum(np.abs(chi_orig)) / norm_r + 1e-30)
