"""Compare parametric forms: error vs EXACT chi (Corradini-PZ).
...
3. Build chi(r) from the two-damped-cosine model
4. FT to q-space via DST-I
5. Compare against exact chi(q) from get_chiq
"""
...
from utils.utils_chi import get_chi0, get_chiq, get_gas_params
...
for form_name in form_names:
    ...
    # Step 2-5: For each rs, interpolate -> build chi(r) -> FT -> compare vs exact
    all_max_q = []
    all_made_q = []
    ...
    for rs in test_rs[4:]:
        ...
        # Build chi(r) on DST grid
        dchi_r = delta_chi(r_grid, rs=rs, params=interp_params)
        chi_interp_r = get_chi0(r_grid, rs) + factor * dchi_r
    ...
    print(f"  worst max|dchiq|/NF = {worst_max:.6f}")
    print(f"  worst MADE = {worst_made:.4f}%,  mean MADE = {mean_made:.4f}%")
    print(f"  ncoeffs = {ncoeffs}")

# Final ranking
print("\n\n" + "=" * 80)
print("RANKING by worst max|δχ(q)|/NF  (vs EXACT chi)")
print("=" * 80)
print(
